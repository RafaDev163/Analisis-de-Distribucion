...
--   ${periodo_inicio}  (ej. 2025-08-01)
--   ${periodo_fin}     (ej. 2025-08-31)
--   ${almacen_codigo}  (ej. 103)
...
FROM PROGRAM $cmd$
  bash -lc "
    set -euo pipefail
    # Ruta del CSV en el contenedor (ajusta si difiere)
    cat /csv/ventasproductosclasificacion.csv
  "
$cmd$
CSV HEADER;
...
WITH params AS (
  SELECT
    COALESCE(NULLIF('${periodo_inicio}',''),'2025-08-01')::date AS periodo_inicio,
    COALESCE(NULLIF('${periodo_fin}',''),'2025-08-31')::date    AS periodo_fin,
    COALESCE(NULLIF('${almacen_codigo}',''),'103')::int         AS almacen_codigo
),
src AS (
  SELECT
